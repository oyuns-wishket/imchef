  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <label className="text-sm font-medium text-ink">조리 순서</label>
      ...
      {steps.map((step, i) => (
        <div key={i} className="flex gap-3 items-start">
          <span className="flex-shrink-0 w-7 h-7 rounded-full bg-ink text-white text-xs font-medium flex items-center justify-center mt-2">
            {i + 1}
          </span>
          <textarea
            value={step.content}
